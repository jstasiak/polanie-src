...
word EMS::SegmentRamki(void) { return segment_okna_EMS; }

word EMS::PodajBlad(void) { return blad; }
...
word EMS::Liczba_uchwytow(void) { return max_licz_uchwytow; }
 ...
 for(byte i = 0; i<liczba_stron_systemu; i++)
  {
   ...
   cout<<dec<<(int)i<<". Segment "<<hex<<semm<<"h czyli "<<dec<<(semm>>6)<<"kB.
Jest to nr strony "<<hex<<strona_fizyczna<<'h'<<endl;
