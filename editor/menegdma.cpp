  ...
  if (!(EMS::Czy_jest_EMS()))
    if (EMS::SegmentRamki())
      if (ile_MAX_chcesz_miec_melodii_WAV)
        if (!Rezerwuj_Pamiec(
                ile_MAX_chcesz_miec_melodii_WAV)) // Zwraca ilosc obiektow
                                                  // OPERWAV
        {
...
/////////////////////////////////////////////////////////////////////////////
// MENEGERDMA::MENEGERDMA(word ile_MAX_chcesz_miec_melodii_WAV,word Kanal,byte
// irq)
/*MENEGERDMA::MENEGERDMA(word ile_MAX_chcesz_miec_melodii_WAV,byte Adlib,byte
jest_SB, byte irq, word port, word kanal)
{
...
/////////////////////////////////////////////////////////////////////////////////
void MENEGERDMA::Init(/*byte Adlib,*/ byte jest_SB, byte irq, word port,
                      word kanal) { // 0x220     0x0083
  // WAVy = NULL;
  Jest_SB = 0;
  return;
  /* if(!WAVy) return;
  // UstawAdlibPresent(Adlib); Przeniesione do funkcji faduj
   ...
   if(EMS::SegmentRamki())
  // if(ile_MAX_chcesz_miec_melodii_WAV)
  // if(Rezerwuj_Pamiec(ile_MAX_chcesz_miec_melodii_WAV)) //Zwraca ilosc
  obiektow OPERWAV if(jest_SB)
   {//TUUUUUUU
    IRQ = irq;
    KanalDMA = port;  //0x220
...
/*void MENEGERDMA::Osiagniecia(void)
{
 ...
 cout<<"\nWersja Sound Blastera "<<dec<<(WersjaSB>>8)<<'.'<<(WersjaSB &
0x00FF)<<endl;
}
  */
//*****************************************
...
word MENEGERDMA::InstalujWAV(char *nazwa) {
  ...
  wolna_str[wolna_strona_fizyczna] =
      1; // Rezewrujemy stone na potrzeby ladowania WAVA

  word rezultat_czytania = NowyWAV.ZaladujWAV_z_dysku(nazwa, WAVy[wolny_uchwyt],
                                                      wolna_strona_fizyczna);

  wolna_str[wolna_strona_fizyczna] =
      0; // Zwracamy rezerwacje - strona wracado puli wolnych
...
word MENEGERDMA::Zwolnij_uchwyt(word numer) {
  if (numer >= max_ilosc_melodii_DMA)
    return PROBA_ZWOLNIENIA_ZBYT_DUZEGO_UCHWYTU;
  if (WAVy[numer].jest_grany)
    return PROBA_ZWOLNIENIA_UCHWYTU_OBECNIE_ODTWARZANEGO;
  if (!(WAVy[numer].zajety))
    return PROBA_ZWOLNIENIA_UCHWYTU_NULL;
...
word MENEGERDMA::Odtwarzaj(word Uchwyt) {
  ...
  if (jest_w_oknie >=
      MAXYMALNA_ILOSC_STRON_FIZYCZNYCH) // Czy muzyka jest juz w oknie
  {
...
Udalo_sie_wpiac_strone:
  ...
  /* asm mov ax,segm
   ...
   asm mov XX,ah
  */
  if ((WAVy[odtwarzany_uchwyt].dlug) ==
      (WAVy[odtwarzany_uchwyt].grana_strona + 1)) {
    ...
    outportb(Rejest_strony_kanalu,Po); // outportb(0x0003,VV); Jak milo, ze nie
    trzeba tego pchac
...
{
// if((Uchwyt >= max_ilosc_melodii_DMA) || (!(pierwszy_OPERWAV[Uchwyt].probki)))
return "NONAME.";
...
void MENEGERDMA::InfoPamiec(void)
{
 ...
 psp = PSP;
   ...
   cout<<"\nSegment PSP : "<<hex<<*(word*)MK_FP(psp-1,1)<<"h w odroznieniu od
prawdziwego PSP: "<<hex<<psp; cout<<"\nWielkosc pamieci :
"<<hex<<koniecPSP<<'h'<<endl;
 ...
 psp = psp + koniecPSP;
   ...
   cout<<"\nZnacznik - "<<hex<<(word)znacznik<<"   Tytul : "<<tytul;
   cout<<"\nSegment PSP : "<<hex<<*(word*)MK_FP(psp,1)<<"h w odroznieniu od
prawdziwego PSP: "<<hex<<psp; cout<<"\nWielkosc pamieci :
"<<hex<<koniecPSP<<'h'<<endl;
