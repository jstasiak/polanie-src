...
#include "zabezset.h"
//=======Zmienne ==============================
void far *memoryBuf[4];
extern void far *fire[14]; //world
extern void far *tree[14]; //world
extern void far *picture[MaxPictures],*missiles[4][3][3],*tlo,*Mysz[13];
extern void far *movers[5][10][3][3],*ramka[4];   //faza:typ:dx:dy
extern void far *buttons[16];
extern void far *Hit[2],*dead[3];
extern void far *shadow;
extern void far *face[16];//0-9 twarze, 10-15 budynki
extern void far *Buttons[4];
extern void far *guzik[3],*lancuch[2];
void far *drewno[3];
//=======Zmienne extern==============================
...
for(int i=0;i<3;i++)
        {
        memoryBuf[i]=NULL;
	memoryBuf[i]=farcalloc(65100L,sizeof(unsigned char));
        }
memoryBuf[3]=NULL;
memoryBuf[3]=farcalloc(24000L,sizeof(unsigned char));

for(i=0;i<4;i++)
        {
        if(memoryBuf[i]==NULL){FreeMemory();return 1;}
        }
buf=(unsigned char far *)memoryBuf[0];
...
buf=(unsigned char far *)memoryBuf[1];
...
buf=(unsigned char far *)memoryBuf[2];
int x,y,faza;
...
buf=(unsigned char far *)memoryBuf[3];

//  postacie   krowa-babcia bez topornika
for(typ=0;typ<4;typ++)         // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
for(faza=0;faza<5;faza++)
for(x=0;x<2;x++)         // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
for(y=0;y<3;y++)          // movers [faza][typ][x][y]
...
for(x=1;x<13;x++)// 1-3
	...
	if(Mysz[x]!=NULL)
            ...
            else {Close13h();cprintf("ERROR 3. Bitmap allocation failed.Mouse.");exit(0);}
        }
...
//przyciski
//Bar13h(254,39,270,53,0);
//Bar13h(254,59,270,73,0);
...
for(x=0;x<10;x++)         // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
 {
  if(face[x]!=NULL)
   GetImage13h(x*32,12*14,(x*32)+32,(12*14)+21,face[x]);
  else {Close13h();cprintf("ERROR 3. Bitmap allocation failed - faces.");exit(0);}
 }
...
for(x=0;x<6;x++)         // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
 {
  if(face[x+10]!=NULL)
   GetImage13h(x*32,12*14,(x*32)+32,(12*14)+21,face[x+10]);
  else {Close13h();cprintf("ERROR 3. Bitmap allocation failed - faces(buildings).");exit(0);}
 }
if(shadow!=NULL)
...
ShowPicture(1,0);
ShowPicture(16,100);
Bar13h(110,32,217,46,0);
Bar13h(110,144,217,158,0);
if(Buttons[0]!=NULL)GetImage13h(108,142,219,160,Buttons[0]);
if(Buttons[1]!=NULL)GetImage13h(108,30,219,48,Buttons[1]);

}
//------------------------------------------------
void FreeMemory()
{
if(memoryBuf[0]!=NULL)farfree(memoryBuf[0]);
if(memoryBuf[1]!=NULL)farfree(memoryBuf[1]);
if(memoryBuf[2]!=NULL)farfree(memoryBuf[2]);
if(memoryBuf[3]!=NULL)farfree(memoryBuf[3]);
}
////////////////////////////////////////////////////////////////////
//
///////////////////////////////////////////////////////////////////
...
if(button)PutImage13h(274,18+20*button,Buttons[2],1);
if(iff+co+max==0)return;
if(iff){PutImageChange13h(278,7,face[co],0,color1,color2);return;}
 else PutImage13h(278,7,face[co],0);
switch(co)
	{
        case 10:
...
if(co<10)
	{
        if(co)PutImage13h(275,39,buttons[0],1);//tarcza
...
void ShowSubMenu(void)
{
int Ty[4]={33,60,90,117};
unsigned char *txt[4]={"Zapisz gr$","Wczytaj gr$","Powt%rz wypraw$","Powr%t do gry"};
...
void ShowBackground(void)
{
odswiez_Polan2(generuj_graf(18)+1);
ShowPicture(3,0);
ShowPicture(18,100);
}
...
do
	...
	k--;
	line[k]=0;
	if(k)OutTextDelay13h(45,10*l+5,line,kolort,kolorb,120);
	l++;
        mouse.GetMsg13h();
...
if(t){Close13h();cprintf("BLAD 303. Brak pamieci operacyjnej. Program wymaga 586kB pamieci podstawowej.");exit(0);}
}
///////////////////////////////////////////////////////////////
...
strcpy(line,"graf\\graf.019");
plik=fopen(line,"r");
if (plik==NULL){Close13h();exit(0);}
k=0;
do
           {
                z=getc(plik);
...
//CenterText13h(1,90,327,104,line,1);
//CenterText13h(2,91,328,105,line,1);
//CenterText13h(2,90,328,104,line,199);
Bar13h(93,120,135+93,134,0);
RisePalette(0);
OutTextDelay13h(45,90,line,199,1,120);
line[0]=0;line[1]=0;
Write13h(93,120,135,11,line,186,0);
*haslo=line[3];
}