...
void InitPicture() {
  ...
  for (x = 0; x < 4; x++)
    if (picture[x + 31] != NULL)
      GetImage13h(11 + 16 * (x + 9), 8 + 126, 27 + 16 * (x + 9), 8 + 140,
                  picture[x + 31]);
    else {
      Close13h();
  ...
  for (x = 0; x < 10; x++) // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
  {
    if (face[x] != NULL)
      ...
      Close13h();
      cprintf("ERROR 3. Bitmap allocation failed - faces.");
      exit(0);
    }
  }
  ...
  for (x = 0; x < 6; x++) // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
  {
    if (face[x + 10] != NULL)
      ...
      Close13h();
      cprintf("ERROR 3. Bitmap allocation failed - faces(buildings).");
      exit(0);
    }
  }
  if (shadow != NULL)
  ...
  if (Buttons[1] != NULL)
    GetImage13h(108, 30, 219, 48, Buttons[1]);
}
//------------------------------------------------
void FreeMemory() {
  if (memoryBuf[0] != NULL)
               ...
               int button) // kto 0-oni 1-myb 2-mypost
{
  ...
  if (button)
    PutImage13h(274, 18 + 20 * button, Buttons[2], 1);
  if (iff + co + max == 0)
    return;
  if (iff) {
    PutImageChange13h(278, 7, face[co], 0, color1, color2);
    return;
  } else
    PutImage13h(278, 7, face[co], 0);
  switch (co) {
  case 10:
    if (max >= 1050)
  ...
  if (co < 10) {
    if (co)
      PutImage13h(275, 39, buttons[0], 1); // tarcza
    if (co == 4 && food > 79) {
...
void ShowSubMenu(void) {
  ...
  unsigned char *txt[4] = {"Zapisz gr$", "Wczytaj gr$", "Powt%rz wypraw$",
                           "Powr%t do gry"};
...
{ // t=0 wstep t=1 zwyciestwo t=2 porazka t=3 koniec)
  ...
  do {
    ...
    line[k] = 0;
    if (k)
      OutTextDelay13h(45, 10 * l + 5, line, kolort, kolorb, 120);
    l++;
    mouse.GetMsg13h();
    if (mouse.Key == 283)
      endd = 1; // esc
...
void ShowPicture(int nr, int b) {
  ...
  if (t) {
    Close13h();
    cprintf("BLAD 303. Brak pamieci operacyjnej. Program wymaga 586kB pamieci "
            "podstawowej.");
    exit(0);
  }
}
